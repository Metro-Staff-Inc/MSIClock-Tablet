...
class BatteryMonitorService {
  ...
  Future<void> initialize() async {
    ...
    // Log initialization
    print(
      'Battery monitoring service initialized at ${DateTime.now().toIso8601String()}',
    );
  }
  ...
  void _scheduleNextReport() {
    ...
    print(
      'Next battery report scheduled in ${duration.inMinutes} minutes and ${duration.inSeconds % 60} seconds',
    );
  }
  ...
  Future<void> _reportBatteryLevel() async {
    ...
    if (_isReporting) {
      print('Battery report already in progress, skipping');
      return;
    }
    ...
    try {
      ...
      if (success) {
        print(
          'Battery report sent successfully: $batteryPct%, MAC: ${macAddress ?? "Unknown"}',
        );
      } else {
        print('Failed to send battery report, will retry later');
        // Schedule a retry in 5 minutes if the report fails
        Timer(const Duration(minutes: 5), _reportBatteryLevel);
      }
    } catch (e) {
      print('Error reporting battery level: $e');
      // Schedule a retry in 5 minutes if the report fails
      Timer(const Duration(minutes: 5), _reportBatteryLevel);
    } finally {
      _isReporting = false;
  ...
  Future<void> triggerManualReport() async {
    print('Manual battery report triggered');
  ...
  void dispose() {
    _reportingTimer?.cancel();
    print('Battery monitoring service disposed');
  }
}
