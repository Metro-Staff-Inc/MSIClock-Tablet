...
class BatteryApiService {
  ...
  Future<bool> sendBatteryCheckIn(BatteryCheckIn checkIn) async {
    try {
      ...
      print('Network error sending battery check-in: $e');
  ...
  void _addToPendingRequests(BatteryCheckIn checkIn) {
    ...
    if (!_pendingRequests.any(
      ...
      print(
        'Added check-in to pending requests queue. Queue size: ${_pendingRequests.length}',
      );
    }
  }
  ...
  Future<void> _retryPendingRequests() async {
    ...
    print('Retrying ${_pendingRequests.length} pending battery check-ins');
    ...
    for (final checkIn in requests) {
      try {
        ...
        if (success) {
          ...
          print(
            'Successfully sent pending check-in. Remaining: ${_pendingRequests.length}',
          );
        }
      } catch (e) {
        print('Error retrying pending check-in: $e');
      }
  ...
  void dispose() {
    _httpClient.close();
    print('Battery API service disposed');
  }
}
